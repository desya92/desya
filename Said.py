...
cl = LINETCR.LINE()
cl.login(token="EmwJDO4TdsHcvTelJ0Ic.ueLeCJvKvdpbtVTadGiEla.VXYnYA+2zZtg6gywocPj3JvccTwpM99Min9et9Lrt4w=")

ki = LINETCR.LINE()
ki.login(token="EmsfHqw8XC76SigA3Qya.TtmrXEUy+dQxPJqjuRhwUG.IrxT/FFt/AKRRjqqz4dLXxkJZnZdBFl7YiTmIqVDpZA=")

kk = LINETCR.LINE()
kk.login(token="EmkBPNezQjBUKIMNmA84.K4ZdSmCEofQe1bhu08sm1a.RWvR4vgD8hyE0kQyboBlvKXz93/MaTTuU/DJB907EI8=")

ks = LINETCR.LINE()
ks.login(token="EmHRjYd8yUjMvvb4a1db.Gl2FmoVBdJvP5zrS8+WsIW.N2RqWkWtTI7pPfnkn5YEzLURJ+uPDO+uj1zjNn+PArg=")

print "login success"
reload(sys)
sys.setdefaultencoding('utf-8')
helpMessage =""" 🇮🇩〘✍Ŧ€₳M☬ж☬Ħ₳ʗҜ฿❂Ŧ✈๛〙🇮🇩
╔══════════════════════  
...
administrator = ["u5049447235645433c14efb400e021e2c"]
admin = ["u5049447235645433c14efb400e021e2c"]
...
#-----------------------------------------------------------   
	        ...
	        elif "As1 " in msg.text:
              #if msg.from_ in admin:
                    ulti0 = msg.text.replace("As1 ","")
                    ulti1 = ulti0.lstrip()
                    ulti2 = ulti1.replace("@","")
                    ulti3 = ulti2.rstrip()
                    _name = ulti3
				...
				if msg.from_ in OWN:
					...
					nk0 = msg.text.replace("Kick2 fuck","")
					nk1 = nk0.lstrip()
					nk2 = nk1.replace("@","")
					nk3 = nk2.rstrip()
					_name = nk3
				...
				if msg.from_ in OWN:
					...
					nk0 = msg.text.replace("Kick3 fuck","")
					nk1 = nk0.lstrip()
					nk2 = nk1.replace("@","")
					nk3 = nk2.rstrip()
					_name = nk3
...
#----------------------------------------------------------
            elif msg.text in ["Respon","Asis"]:
                ki.sendText(msg.to,"▒▓█►✍Bot1🇮🇩◄█▓▒ ")
                kk.sendText(msg.to,"▒▓█►✍Bot2🇮🇩◄█▓▒ ")
                ks.sendText(msg.to,"▒▓█►✍Bot3🇮🇩◄█▓▒ ")		
#----------------------------------------------------------

            elif msg.text == "Sider":
                    cl.sendText(msg.to, "Cctv check ( ^ω^)")
                    try:
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "Ketik Sider dulu dudul Baru bilang Read.")
#-----------------------------------------------------------speed
...
#-----------------------------------------------
        ...
        if op.type == 17:
            ...
            cl.sendText(op.param1, "Welcome to 👉 owner group ( ^ω^) " + str(ginfo.name) + " :\n" + ginfo.creator.displayName)
...
#-----------------------------------------------------------
        ...
        if op.type == 15:
                if op.param2 in Bots:
                     return
                cl.sendText(op.param1, "Bye-bye Terima kasih sudah gabung ( ^ω^)")
